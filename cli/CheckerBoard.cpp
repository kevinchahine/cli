...
namespace cli
{
	namespace boards
	{
		...
		void CheckerBoard::placePiece(char piece, int row, int col, bool isWhite)
		{
			int r = 2 + cellSize.rows() * row + (cellSize.rows() / 2);
			int c = 2 + cellSize.cols() * col + (cellSize.cols() / 2);

			(*this)[r][c].character = piece;
			(*this)[r][c].color.setfg(isWhite ? lightPiece : darkPiece);
		}

		void CheckerBoard::placePiece(char piece, const Point & pos, bool isWhite)
		{
			placePiece(piece, pos.row, pos.col, isWhite);
		}
		...
		void CheckerBoard::print(std::ostream & os, size_t nTabs) const
		{
			this->Matrix::print();
		}
		...
		void CheckerBoard::drawBoarder()
		{
			...
			rectangle(*this, cli::Point{ 0, 0 }, this->size(), color, true);
		}
